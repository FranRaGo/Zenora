...
const TaskColumn = ({ status, project }) => {
                ...
                <div className="div-status" ref={provided.innerRef} {...provided.droppableProps}>
                    ...
                    {dropdown && (
                        <>
                            ...
                            <div className="task-list">
                                {tareasFiltradas.map((tarea, index) => (
                                    <Draggable key={tarea.id} draggableId={tarea.id.toString()} index={index}>
                                        {(provided) => (
                                            <div className="tasks-item" ref={provided.innerRef} {...provided.draggableProps}>
                                                <div className="task-drag-select">
                                                    ...
                                                    <input type="checkbox" id="checkbox-tasks"/>
                                                </div>

                                                <TaskItem tarea={tarea} index={index} disableDraggable />
                                            </div>
                                        )}
                                    </Draggable>
                                ))}
                            ...
                            {tareasFiltradas.length === 0 && <button id="add-new-task">+ Añadir tareas</button>}
                        </>
                    )}
                </div>
            )}
